...
namespace unlockfpsclr
{
	...
	Void SettingsForm::OnLoad(Object^ sender, EventArgs^ e)
	{
		...
		customResX->Value = Convert::ToDecimal(std::clamp(settings->CustomResX, Convert::ToInt32(customResX->Minimum),
		                                                  Convert::ToInt32(customResX->Maximum))); // sanitization
		customResY->Value = Convert::ToDecimal(std::clamp(settings->CustomResY, Convert::ToInt32(customResY->Minimum),
		                                                  Convert::ToInt32(customResY->Maximum))); // sanitization
		monitorNum->Value = Convert::ToDecimal(std::clamp(settings->MonitorNum, Convert::ToInt32(monitorNum->Minimum),
		                                                  Convert::ToInt32(monitorNum->Maximum))); // sanitization
	...
	Void SettingsForm::btnDllAdd_Click(Object^ sender, EventArgs^ e)
	{
		...
		if (dialog->ShowDialog() == Windows::Forms::DialogResult::OK)
		{
			...
			if (auto index = lbDllList->Items->IndexOf(selectedFile); index >= 0)
			{
				...
				MessageBox::Show("File already in list.", "Genshin Impact FPS Unlocker", MessageBoxButtons::OK,
