	...
	Void SettingsForm::btnDllAdd_Click(Object^ sender, EventArgs^ e)
	{
		...
		if (dialog->ShowDialog() == System::Windows::Forms::DialogResult::OK)
		{
			...
			if (auto index = lbDllList->Items->IndexOf(selectedFile); index >= 0)
			{
				...
				MessageBox::Show("File already in list", "Genshin FPS Unlocker", MessageBoxButtons::OK, MessageBoxIcon::Information);
				return;
			}
			...
			{
				MessageBox::Show("Invalid File\nThe DLL is either corrupted or 32-bit\nOnly 64-bit DLLs are supported", "Genshin FPS Unlocker", MessageBoxButtons::OK, MessageBoxIcon::Error);
				return;
			}
