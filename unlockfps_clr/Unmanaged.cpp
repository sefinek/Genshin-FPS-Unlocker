...
bool Unmanaged::SetupData()
{
	...
	if (nt->FileHeader.TimeDateStamp < 0x645B245A) // < 3.7
	{
		...
		*/
		...
		if (!address)
			return MessageBoxA(nullptr, "Outdated fps pattern. [1]\n\nSad cat...", "Genshin FPS Unlocker", MB_OK | MB_ICONERROR) == -1 && VirtualFree(up, 0, MEM_RELEASE) == -1; // Lazy returns, should always evaluate to false
	...
	else
	{
		...
		if (nt->FileHeader.TimeDateStamp < 0x656FFAF7) // < 4.3
		{
			...
			if (!address)
				return MessageBoxA(nullptr, "[1.235]: Outdated fps pattern.", "Error", MB_OK | MB_ICONERROR) == -1 && VirtualFree(up, 0, MEM_RELEASE) == -1;
		...
		else
		{
			...
			if (!address)
				return MessageBoxA(nullptr, "[2.245]: Outdated fps pattern.", "Genshin FPS Unlocker", MB_OK | MB_ICONERROR) == -1 && VirtualFree(up, 0, MEM_RELEASE) == -1;
